...
from sql_queries import insert_table_queries, create_table_queries, drop_table_queries
import warnings
...
def read_anime_df(df, suffix, BUCKET_NAME, path, s3c):
    ...
    df_anime = pd.DataFrame(columns=['animeID','title','title_english','type','source','episodes','status','duration',
                                      'rating','score','scored_by','members','favorites','aired_from', 
                                      'aired_to','premiered','broadcast'])
    df_relations = pd.DataFrame(columns=['animeID', 'relation', 'id_relation','relation_type'])
    df_producers = pd.DataFrame(columns=['animeID', 'producer_id', 'producer_name'])
    df_licensors = pd.DataFrame(columns=['animeID', 'licensor_id', 'licensor_name'])
    ...
    for i, row in df.iterrows():
        ...
        if aux_aired_to != None:
            aired_to = parse(aux_aired_to).date()
        else:
            aired_to =aux_aired_to

        #Append trasformed data into df_anime
        df_anime = df_anime.append(pd.DataFrame([[row.animeID, row.title, row.title_english, row.type, 
                                                  row.source, row.episodes, row.status, row.duration, 
                                                  row.rating, row.score, row.scored_by, row.members,
                                                  row.favorites, aired_from, aired_to, row.premiered, 
                                                  row.broadcast]], 
                                      columns=['animeID','title','title_english','type','source','episodes','status',
                                               'duration','rating','score','scored_by','members',
                                               'favorites','aired_from','aired_to','premiered','broadcast']),
                             ignore_index=True)
        ...
        for relation in aux_relations:
            ...
            for j in range(0,len(aux_relations[relation])):
                ...
                df_relations = df_relations.append(pd.DataFrame([[row.animeID, relation,aux_relations[relation][j]['mal_id'],
                                                    aux_relations[relation][j]['type']]], 
                                      columns=['animeID', 'relation','id_relation','relation_type']),
                             ignore_index=True)
        ...
        for j in range(0,len(aux_producers)):
            ...
            df_producers = df_producers.append(pd.DataFrame([
                [row.animeID, aux_producers[j]['mal_id'], aux_producers[j]['name']]], 
                                    columns=['animeID', 'producer_id', 'producer_name']),ignore_index=True)
        ...
        for j in range(0,len(aux_licensors)):
            ...
            df_licensors = df_licensors.append(pd.DataFrame([
                [row.animeID, aux_licensors[j]['mal_id'], aux_licensors[j]['name']]], 
                                    columns=['animeID', 'licensor_id', 'licensor_name']),ignore_index=True) 
        ...
        for j in range(0,len(aux_studios)):
            ...
            df_studios = df_studios.append(pd.DataFrame([
                [row.animeID, aux_studios[j]['mal_id'], aux_studios[j]['name']]], 
                                    columns=['animeID', 'studio_id','studio_name']),ignore_index=True)
    
        #Transform genre string into list    
        aux_genres = literal_eval(row.genre)
        for j in range(0,len(aux_genres)):
            #Append trasformed data into df_studios
            df_genres = df_genres.append(pd.DataFrame([
                [row.animeID, aux_genres[j]['mal_id'], aux_genres[j]['name']]], 
                                    columns=['animeID', 'genre_id','genre_name']),ignore_index=True)
    
    
    #Transform df to csv and stores it into S3
    copy_csv_to_s3(client=s3c, df=df_anime, bucket=BUCKET_NAME, filepath= path + 'animes_' + str(suffix) + '.csv')
    copy_csv_to_s3(client=s3c, df=df_relations, bucket=BUCKET_NAME, filepath= path + 'anime_relations_' + str(suffix) + '.csv')
    copy_csv_to_s3(client=s3c, df=df_producers, bucket=BUCKET_NAME, filepath= path + 'producers_' + str(suffix) + '.csv')
    copy_csv_to_s3(client=s3c, df=df_licensors, bucket=BUCKET_NAME, filepath= path + 'licensors_' + str(suffix) + '.csv')
    copy_csv_to_s3(client=s3c, df=df_studios, bucket=BUCKET_NAME, filepath= path + 'studios_' + str(suffix) + '.csv')
    copy_csv_to_s3(client=s3c, df=df_genres, bucket=BUCKET_NAME, filepath= path + 'genres_' + str(suffix) + '.csv')
...
def read_AnimeList_df(df, KEY, BUCKET_NAME, path, s3c):
    ...
    #Since there is not an column with user name on json file, it's used part of the file name to identify it.
    username = file[0:file.rfind('_')]
    
    # Create data frame structure of staging table
    df_AnimeList = pd.DataFrame(columns=['username', 'animeID', 'watching_status','score', 'watched_episodes', 
                                     'is_rewatching','watch_start_date','watch_end_date','days','priority'])
    ...
    for i, row in df.iterrows():
        ...
        for j in range(0,len(row.anime)):
            ...
            df_AnimeList = df_AnimeList.append(pd.DataFrame([
                [username, row.anime[j]['mal_id'], row.anime[j]['watching_status'],
                 row.anime[j]['score'],row.anime[j]['watched_episodes'], row.anime[j]['is_rewatching'],
                 watch_start_date, watch_end_date,row.anime[j]['days'],row.anime[j]['priority']]],
                    columns=['username', 'animeID', 'watching_status', 'score','watched_episodes',
                             'is_rewatching','watch_start_date','watch_end_date','days','priority']),ignore_index=True)
    
    #Transform df to csv and stores it into S3
    copy_csv_to_s3(client=s3c, df=df_AnimeList, bucket=BUCKET_NAME, filepath= path + 'staging_' + file)
...
def read_user_df(df, KEY, BUCKET_NAME, path, s3c):
    ...
    df_user = df[['user_id', 'username', 'last_online', 'gender', 'birthday', 'location', 'joined']]
    df_user['birthday'] = pd.to_datetime(df_user['birthday'], format='%Y-%m-%d')
    df_user['joined'] = pd.to_datetime(df_user['joined'], format='%Y-%m-%d')
    df_user['last_online'] = pd.to_datetime(df_user['last_online'], format='%Y-%m-%d %H:%M:%S')
    ...
    for i, row in df.iterrows():
        ...
        for j in range(0,len(aux_favorite)):
            ...
            df_favorite_anime = df_favorite_anime.append(pd.DataFrame([
                [row.username, aux_favorite[j]['mal_id']]],
                    columns=['username', 'animeID']),ignore_index=True)
    
    #Transform df to csv and stores it into S3
    copy_csv_to_s3(client=s3c, df=df_favorite_anime, bucket=BUCKET_NAME, filepath= path + 'staging_favorite_' + file)
    copy_csv_to_s3(client=s3c, df=df_user, bucket=BUCKET_NAME, filepath= path + 'users_' + file)
...
def copy_tables_redshift(cur, conn, ARN, path, file_format, copy_to, copy_from = '', IGNOREHEADER = True):
    """
...
def count_query(cur, conn, table):
    """
...
def main():
    ...
    """
    all_tables = ['animes','anime_relations','producers','licensors','studios','genres','staging_anime_list','staging_favorite', 'users']
    ...
    anime_keys = []
    for key in get_matching_s3_keys(bucket=BUCKET_NAME, prefix=ANIME_INPUT, s3c = s3c):
    ...
    for count in range(len(anime_keys)):
        ...
        df = pd.read_csv(io.BytesIO(obj['Body'].read()), sep = ',' , delimiter=",", engine = 'python', encoding='utf-8')
    ...
    animelist_keys = []
    for key in get_matching_s3_keys(bucket=BUCKET_NAME, prefix=ANIME_LIST_INPUT, suffix = '.json', s3c = s3c):
    ...
    users_keys = []
    for key in get_matching_s3_keys(bucket=BUCKET_NAME, prefix=USER_INPUT, suffix = '.json', s3c = s3c):
    ...
    print(F"\nTransform RAW data process took {str(quality_data_time - transform_raw_data_time)} seconds\n")

    #----------------------------------------------------------------------------------------------------
    #Initiate data quality check
    print('------------------')
    ...
    for table in all_tables:
        ...
        if count_s3[i] == 0:
            ...
            count_redshift = count_query(cur, conn, table)
            ...
            if count_redshift != 0:
                ...
                if delta_data < 99:
                    print(F"Data quality fail because S3 table '{table}' has {str(delta_data)}% data compared to Redshift table (Acceptable loss of 1%).")
                    print('------------------')
                    data_quality_pass = False
                else:
                ...
                print(F"Redshif table '{table}' is empty. S3 '{table}' has passed data quality check.")
                print('------------------')
        i += 1
    
    redshift_data_time = time.time()
    print(F"Data quality process took {str(redshift_data_time - quality_data_time)} seconds")
                                       
    if not data_quality_pass:
        conn.close()
        print ("Not all tables passed data quality check.\nTerminating ETL process.")
        return None
    else:
    ...
    copy_table = ['animes','anime_relations','producers','licensors', 'studios','genres']
    for table in copy_table:
        copy_tables_redshift(cur = cur, conn = conn, ARN = ARN, path = ANIME_OUTPUT, file_format = 'csv', copy_to = table, copy_from = table)
        
    copy_tables_redshift(cur = cur, conn = conn, ARN = ARN, path = ANIME_LIST_OUTPUT, file_format = "csv", copy_to = 'staging_anime_list')
    
    copy_table = ['staging_favorite','users']
    for table in copy_table:
        copy_tables_redshift(cur = cur, conn = conn, ARN = ARN, path = USER_OUTPUT, file_format = "csv", copy_to = table, copy_from = table)
    ...
    print(F"Redshift process took {str(time.time() - redshift_data_time)} seconds\n")
